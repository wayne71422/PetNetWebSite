...
$(document).ready(function () {
    const species = document.getElementById('SpeciesId');
    console.log(species.value);
    const breed = document.getElementById('BreedId');
    if (species) {

        CreateBreedList(species.value, breed)
...
function CreateBreedList(speciesVal, breed) {
    if (speciesVal == '') {
        let option = document.createElement('option');
        option.textContent = '請先選擇種類';
        option.value = '';
        breed.appendChild(option);
    }
    else {
        const list = speciesVal == 'C' ? catList : dogList;
        list.forEach(member => {
            console.log(breed.value);
            if (breed.value != member) {
                let option = document.createElement('option');
